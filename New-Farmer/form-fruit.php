...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit"]) && !empty($_POST))
{
 ...
 mysqli_query($conn,"SET NAMES utf8;");   
    $id="SELECT * from fruit_name where crop = '$cr'";
    $run = mysqli_query($conn,$id);
    $crres=mysqli_fetch_assoc($run);
    /*.....*/
    /*FETCHING CATOGoRY AND ID*/
    $qctg="SELECT ctg_id , category from category where category LIKE 'fruits'";
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title" id="reqcropmes">Add New Fruit Name</h4>
        </div>
        <div class="modal-body">
          <form method="POST">
            <div class="form-group">
              <label for="recipient-name" class="form-control-label formlabel"><strong>New Fruit Name</strong></label>
              <input type="text" class="form-control" id="reqfruit" name="reqfruit" />
            </div>
        </div>
